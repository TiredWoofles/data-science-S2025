...
- Are there any sources of *real* uncertainty in the percent occupied
  you calculated?
  - Yes, as someone could have died or moved out
- Are there any sources of *erroneous* uncertainty in the percent
  occupied you calculated?
  - Yes, as I may have miscounted or rounding errors
...
df_q3 <- df_sample_seq %>% 
  mutate(last = str_extract(name, "\\w+$")) %>%  # Extract last word as last name
  count(last) %>% 
  mutate(p = n / sum(n)) %>% 
  arrange(desc(p))
...
- Which sample—sequential or random—is more *representative* of all
  ...
  - Random is more representative of all homes because families living
    next to each other could have the same name due to families living
    closer together
...
name_prevalence <- function(df, last = "Collins") {
  df %>% 
    ...
    summarize(estimate = mean(last_name == last)) %>% 
...
df_interval_bootstrap <- 
  ...
  mutate(
    estimate = map(
      splits,
      ~ analysis(.x) %>% name_prevalence(last = "Collins")
    )
  ) %>% 
  int_pctl(estimate)
df_interval_bootstrap
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - df_sample_seq is not representative of the data the bootstrap
    interval should not be applied.
...
- Arcadia University students studying
...
- (Observation or experiment?)
- Which majors can perform the best physically and mentally when under
  the influence of alcohol?
- 1.Before alcohol Measure BAC
- 2.Before alcohol have them do coordination test (balance eyes closed)
- 3.Before alcohol have them do a mental test (memory game)
- 3.Take 6 shots of 30mL vodka
- 4.After alcohol measure BAC
- 5.After alcohol have them do coordination test
- 6.After alcohol have them do a mental test

#### Question / Hypothesis

- (Write your question / hypothesis here)

  “What effect does alcohol have on different majors in the Arcadia
...
- (What steps will you take to collect the data?)
  ...
  - Each time member arts (Suwanee), Michaela (Law), Ariel (Science),
    Health (Allyson), Business (Lucien)
- (How will you ensure the data is representative of your chosen
  population?)
- We plan to pick 25 randomly from each major
- (For experiments only: How will you ensure any effects you observe are
  due to the treatment, and not due to other factors?)
- BAC level to measure alcohol has changed and we wont be giving them
...
- (How will you choose your sample size?)
  ...
  - We chose a sample size of 25 as our majors are about the size of
    50ish which is half

...
- You chose a correct value of `stat(x, y)` when `x, y` is *outside* the
  ...
  - Points outside the circle (x<sup>2</sup> + y^2 less than equal to 1)
    , we return 4 because indicator (4) should equal pi
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  circle. Why did you choose this value?
  - Outside points should return 0 so they add to our pi estimation
...
- How closely do your bootstrap CI and CLT CI agree?
  - They are within 0.1 of each other.
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - Yes because i can always buy a little more if i need to
  - No it would likely not as rocket trajectories are very precise very
    precise and involve more at stake resulting in percision to a higher
    amount of sig figs.
- What would be a *valid* way to make your CI more narrow?
  - Increase the sample size n and use variance reduction

...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   No, because the sample is based on sequential sampling, meaning houses are sampled in a sequence, likely in geographical proximity. As a result, certain groups that tend to live farther apart (e.g., wealthy people often live in separate areas from others) may not be included in the sample. This sampling method might not capture the diversity of the entire population.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    ...
    -   Random is more representative of all homes because families living next to each other could have the same name due to families living closer together
...
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   df_sample_seq is not representative of the data the bootstrap interval should not be applied.
...
-   (Observation or experiment?)
-   Which majors can perform the best physically and mentally when under the influence of alcohol?
...
-   (Write your question / hypothesis here)
    ...
    "What effect does alcohol have on different majors in the Arcadia University Population?"
...
-   (What steps will you take to collect the data?)
    ...
    -   Each time member arts (Suwanee), Michaela (Law), Ariel (Science), Health (Allyson), Business (Lucien)
...
-   BAC level to measure alcohol has changed and we wont be giving them any additional tests
-   (How will you choose your sample size?)
    ...
    -   We chose a sample size of 25 as our majors are about the size of 50ish which is half

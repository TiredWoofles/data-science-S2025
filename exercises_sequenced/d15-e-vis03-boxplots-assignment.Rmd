...
The Minivan tends to be the middle. The boxplot is far more effective at this.
...
```{r}
diamonds %>%
  mutate(carat_cut = cut_number(1:53940, 5)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```
...
```{r}
diamonds %>%
  mutate(carat_cut = cut_interval(1:53940, 5)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```
